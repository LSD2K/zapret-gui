...
> ### ⚠️ Этот файл больше не подаётся агенту целиком
>
> Он весит ~104 КБ (**~30K токенов**) — четверть рабочего окна ещё до
> первой строки кода. Для работы план **нарезан по сессиям**:
>
> - [`docs/mcp/00-contract.md`](mcp/00-contract.md) — общие правила, читаются
>   в каждой сессии (~1.5K токенов);
> - `docs/mcp/NN-*.md` — по файлу на сессию, читается только своей сессией;
> - [`docs/mcp/HANDOFF.md`](mcp/HANDOFF.md) — эстафета: что сделано, какие
>   контракты зафиксированы, где подключиться следующему;
> - [`docs/mcp-implementation-roadmap.md`](mcp-implementation-roadmap.md) —
>   порядок сессий, зависимости, бюджет контекста.
>
> Сам план остаётся **источником и архивом**: сюда ходят точечным `grep`
> за деталями (примеры обмена, приложения, обоснования решений), на которые
> задания ссылаются по номерам разделов. Правки замысла вносятся сюда,
> а затем — в затронутое задание.
