...
Форк используется только как движок под своей панелью `LSD2K/gw-panel` (REST на 10.10.10.1:8080:
nfqws2, стратегии, hostlists, sing-box конфиги, agh_routes). Его веб-интерфейс владельцу не
нужен. Ужимание по `spec-t1-trim.md` остановлено на полпути (правки в ветку не попали),
возобновлять только по запросу владельца. Тег `v0.25.3-gw.2`.
