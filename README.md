...
- **По каталогу zapret-gui** — наш перебор: ранжирует приёмы от простых к
  сложным, найденное применяется в один клик.
...
> **По умолчанию всё выключено.** Точка отдаёт 404, токен пуст, все
> одиннадцать разрешений сняты. Пока вы не включили сервер руками и не
> выдали разрешения, ничего из описанного ниже не работает — и это
> сознательное состояние по умолчанию, а не недонастроенность.
...
2. **«Сгенерировать токен»** — значение показывается **один раз**,
   ...
   клиентов**;
3. **Разрешения** — одиннадцать переключателей, у каждого написано, чем
...
93 инструмента. Что из них видно клиенту — решают разрешения: невыданное
не просто запрещено, оно **не публикуется**, и модель о нём не знает.

**Чтение — всегда, отдельного разрешения нет** (32):
...
**`strategies_write` — стратегии и списки** (8): `strategy_save`,
`strategy_delete`, `strategy_compose` (описание → argv),
`strategy_validate` (прогон `nfqws2 --intercept=0`), `hostlist_edit`,
`ipset_edit`, `blob_add`, `lua_script_save`.
...
**`probes` — трафик наружу** (8): `probe_targets`, `probe_compare`
...
**`dangerous`** (1): `system_reboot` — и только через двухшаговое
подтверждение.
...
**`tunnels_write`** — переключатель есть, инструментов за ним пока нет:
конфиги туннелей, подписки и правила единого слоя на запись ещё не
вынесены в MCP. Включать его сегодня бессмысленно.
...
> `code_export_patch` (или кнопка «Патч» на странице MCP) — и заведите
