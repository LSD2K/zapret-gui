...
> двенадцать разрешений сняты. Пока вы не включили сервер руками и не
...
опроса**. Только на нём работает подписка на ресурсы
...
потока, а не молчаливым «принято».
...
116 инструментов. Что из них видно клиенту — решают разрешения:
невыданное не просто запрещено, оно **не публикуется**, и модель о нём
не знает.

**Чтение — всегда, отдельного разрешения нет** (36):
...
- **инструментов отдаётся столько, сколько модель осилит.** 116
