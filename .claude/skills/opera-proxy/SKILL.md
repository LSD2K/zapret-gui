...
1. **Сам бинарник.** `opera-proxy -h` — единственный достоверный список флагов
   ...
   релиза (`v1.29.0`). Всё в §3 сверено с выводом `-h` реальной сборки
   `opera-proxy.linux-amd64` v1.28.0 и остаётся верным для **v1.29.0**: между
   этими релизами в апстриме поменялись ТОЛЬКО `go.mod`/`go.sum` (обновление
   зависимостей), ни одного изменения в коде и флагах.
...
## 3. CLI-флаги (сверено с `-h` v1.28.0; в v1.29.0 не изменились)
...
| Флаг | Где используем |
|------|----------------|
| `-version` | `_get_version()` — печатает ровно тег (`v1.29.0`), на этом держится сравнение «уже актуальная версия» в установщике и `has_update` в «Обновлениях» |
...
"pinned_tag": "v1.29.0",    # версия, для которой известны sha256
"allow_unpinned": True,
