...
1. **Сам бинарник.** `opera-proxy -h` — единственный достоверный список флагов
   ...
   релиза (`v1.30.0`). Всё в §3 сверено с выводом `-h` реальной сборки
   `opera-proxy.linux-amd64` и остаётся верным для **v1.28.0 → v1.30.0**: в
   этих релизах в апстриме поменялись ТОЛЬКО `go.mod`/`go.sum` (обновление
   зависимостей), ни одного изменения в коде и флагах. Сверка v1.30.0 сделана
   прогоном `-h` на скачанной сборке: 46 флагов, набор совпадает с v1.28.0.
   Единственное следствие обновления зависимостей, которое нас касается, —
   `golang.org/x/crypto/x509roots/fallback` (см. ниже врезку о корневых
   сертификатах).
...
## 3. CLI-флаги (сверено с `-h` v1.28.0; в v1.29.0 и v1.30.0 не изменились)
...
| Флаг | Где используем |
|------|----------------|
| `-version` | `_get_version()` — печатает ровно тег (`v1.30.0`), на этом держится сравнение «уже актуальная версия» в установщике и `has_update` в «Обновлениях» |
...
> **Корневые сертификаты едут внутри бинарника.** `main.go` делает пустой
> импорт `golang.org/x/crypto/x509roots/fallback`: если в системе нет
> хранилища CA — а на Entware без `ca-certificates` его как раз нет, —
> доверие к `api2.sec-tunnel.com` и узлам `*.sec-tunnel.com` держится на
> этом вшитом наборе. Поэтому обновление зависимостей у апстрима не всегда
> «просто бамп»: v1.30.0 не тронул ни одного флага, но обновил именно этот
> набор корней (и `go-socks5` — слушатель `-socks-mode`, и `x/net`). Если
> после обновления бинарника вылезает `x509: certificate signed by unknown
> authority` — лечится не флагом обхода, а `-cafile` со своим бандлом или
> установкой `ca-certificates`.
...
"pinned_tag": "v1.30.0",    # версия, для которой известны sha256
"allow_unpinned": True,
