...
2. **`nfqws2 -?`** (v1.0.5.2, актуальный релиз на 2026-09) — дамп опций CLI, §3 ниже.
...
**Актуальный релиз — v1.0.5.2** (2026-09-15). Предыдущий ориентир этого
справочника был v1.0.4 (2026-07-31), а до него — v0.9.5.2 (2026-04-30).

**Главное для нас: в 1.0.5 впервые с 0.9.5.2 появились НОВЫЕ CLI-опции** —
`--filter-mark` (фильтр профиля по mark пакета) и три инверсии сетевых
фильтров (`--filter-ssid-neg`, `--ssid-filter-neg`, `--nlm-filter-neg`).
До этого набор опций не менялся вовсе, кроме переименования `--writeable` →
`--writable` в 1.0. Набор C-функций Lua (102 регистрации в `nfq2/lua.c`) и
таблицы `--filter-l7` / `--payload` по-прежнему те же — семейство
`dissect_tcphdr`/`dissect_iphdr`/… в 1.0.5 не добавили, а лишь
ЗАДОКУМЕНТИРОВАЛИ (в коде оно было и в 1.0.4). То есть §3–§7 остаются
валидными с поправкой на четыре новые опции; менялись семантика таймеров,
`send:delay`, поведение при недоступном blob, реасм, а в 1.0.5 — ещё и
паддинг TCP-опций и возвращаемое значение desync-функции (см. 0.1).
...
| **1.0.5** | 2026-09-02 | **Новые опции: `--filter-mark=mark[/mask]`** (фильтр профиля по mark пакета) и **инверсии фильтров** `--filter-ssid-neg`, `--ssid-filter-neg`, `--nlm-filter-neg`. **desync-функция может явно вернуть `nil` — это синоним `VERDICT_PASS`** (раньше явный nil был ошибкой). `zapret-lib: luaexec` теперь ПРОБРАСЫВАЕТ вердикт вызванного кода (раньше терял). **TCP-опции: `reconstruct` паддит нулями, `dissect` срезает хвостовые `0x00`** — реконструкция перестала зависеть от того, был ли в исходнике EOL-паддинг. blockcheck2: в Windows вместо `--ipset` используется `--wf-raw-filter`; lua поднят до 5.5.1. |
| **1.0.5.1** | 2026-09-07 | Оптимизация поиска профиля (сначала быстрые проверки). Поведение не меняется. |
| **1.0.5.2** | 2026-09-15 | Ещё оптимизации поиска профиля. Плюс (в диапазоне после 1.0.5.1, но без записи в `changes.txt`): автохостлисты больше не становятся «непустыми», если ни один список не перечитался, и фикс выхода по `HUP`. |

### 0.2 Что из этого меняет наши инварианты

1. **Совместимость lua↔бинарник — по-прежнему `NFQWS2_COMPAT_VER`, сейчас 6.**
   Наши bundled-lua (`import/lua/zapret-lib.lua:1`) уже на 6 — паритет с
   1.0.x есть.
2. ⚠️ **Ловушка `_protected_core_lua`.** `core/asset_importer.py` защищает
   core-lua на диске, только если её `NFQWS2_COMPAT_VER_REQUIRED` **строго
   больше** нашей bundled. Внутри одной compat-версии (все 1.0…1.0.4 = 6)
   защиты нет — bundle затирает upstream-копию. То есть отставший бандл
   молча откатывает пользователю апстримный фикс, не изменив ни одной
   проверяемой версии.
   **Единственная защита — держать core-lua дословной копией релиза.**
   Сейчас все шесть файлов (`zapret-lib`, `zapret-antidpi`, `zapret-auto`,
   `zapret-obfs`, `zapret-pcap`, `zapret-tests`) побайтово равны **v1.0.5.2**;
   sha256 запиннены в `tests/test_asset_importer_lua_compat.py`
   (`_UPSTREAM_LUA_TAG` / `_UPSTREAM_LUA_SHA256`), там же — инструкция по
   обновлению на новый релиз. Наши собственные функции живут в отдельных
   файлах (`custom_funcs.lua`, `zapret-multishake.lua`, …) — core-lua
   правится **только** синхронизацией с апстримом.
   Насколько это не теория: бандл на v1.0.4 откатывал пользователю
   1.0.5-фиксы `zapret-lib` — проверку длины в `tls_dissect_ext` для
   расширения `key_share` (`left>=1` → `left>=4`), длину random в
   `tls_dissect_handshake` (36 → 38), реконструкцию ClientHello **без**
   расширений (раньше в неё всё равно писалась 2-байтовая длина списка) и
   проброс вердикта из `luaexec`. Ни одна версия при этом не менялась:
   compat остаётся 6.
3. **`--intercept=0` + таймеры** — см. §12.1: валидация больше не гарантирует
   мгновенный выход.
4. **Недоступный blob** (1.0.4) — инстанс сносится в C-коде, до Lua дело не
   доходит. `--intercept=0` этого по-прежнему не ловит (блобы резолвятся
   по-пакетно), но в `--debug` теперь видно раньше и однозначнее.
5. **Discord UDP 19294-19344** — апстрим догнал наш дефолт
...
## 3. Полный CLI nfqws2 (`nfqws2 -?`, v1.0.5.2)

Дамп v1.0.5.2 движка `bol-van/zapret2`. Версия печатается:
`github version v1.0.5.2 (<git-hash>) lua_compat_ver 6`. Опции с `[0|1]`/
`[=val]` имеют необязательный аргумент (без него — дефолт, обычно `=1`).

**Набор опций совпадает с v0.9.5.2**, кроме переименования `--writeable` →
`--writable` (1.0) и **четырёх опций, добавленных в 1.0.5**: `--filter-mark`,
`--filter-ssid-neg`, `--ssid-filter-neg`, `--nlm-filter-neg`. Всё, что
собирает GUI, на старых сборках работает без изменений — новые опции
опциональны и никем из наших генераторов не ставятся.
...
| `send` | Отправить текущий диссект (без дропа оригинала). args: `dir`, `fooling`, `ipid`, `ipfrag`, `reconstruct`, `rawsend`, `delay=<мс>`. Дефолт `ip_id=none`. Вердикт всегда **PASS** — оригинал уйдёт следом; чтобы отменить, добавь отдельный инстанс `drop`. См. §8.9 про `delay`. |
...
### 8.9 Отложенная отправка `send:delay=<мс>` (с zapret2 1.0)

`--lua-desync=send:delay=200` не блокирует обработку: диссект и опции
отправки запоминаются, ставится **однократный таймер**, отправка происходит
по его срабатыванию. Ключевые следствия:

- Имя таймера уникально на **диссект + инстанс** (`desync_timer_name()`;
  включение имени инстанса добавили в 1.0.4). Поэтому **повторный
  `send:delay` по тому же потоку не добавляет вторую отправку, а замещает
  предыдущую** — очереди нет. Нужна строгая очередь нескольких вариаций
  (например результат TCP-сегментации) — пиши свою функцию, `send` не
  подойдёт.
- Вердикт остаётся `VERDICT_PASS`: оригинал уходит немедленно. Задержать
  сам оригинал — только связкой `drop` + собственная логика.
- `sleep` в Lua нет и не будет: обработка однопоточная, любая пауза вешает
  всю очередь. Всё асинхронное — через таймеры (§19.5).

### 8.10 Очередь отложенных пакетов реасма (лимит 64, с 1.0.3)

Пока nfqws2 собирает многопакетный пейлоад (kyber-`tls_client_hello`,
многопакетный `quic_initial`), исходные пакеты копятся в очереди conntrack —
в логе `DELAY desync until reasm is complete (#N)`. С 1.0.3 очередь
ограничена **64 пакетами** (`RAW_PACKET_QUEUE_DELAYED_MAX`, анти-DDoS).

При переполнении в `--debug` видно `rawpacket_queue failed !`; nfqws2
отменяет реасм, сбрасывает `l7payload` в `unknown` и уходит на повторный
подбор профиля. То есть **стратегия на `tls_client_hello` просто перестаёт
применяться на этом соединении** — типичная причина «работало, а на одном
сайте нет». Смотреть в лог именно эту строку.
...
- `NFQWSManager.dry_run(strategy_args)` собирает argv тем же
  ...
  и любые `--intercept`/`--dry-run`, добавляет **`--intercept=0`** (эталон:
  «выполнить только lua-init и выйти»), запускает и проверяет `returncode==0`.
  NFQUEUE не открывается, трафик не затрагивается.
- Ловит: кривой парсинг опций CLI, отсутствующие файлы
  (`--blob`/`--lua-init`/`--hostlist`/…) И **ошибки lua-init** — синтаксис
  lua-скриптов и ошибки времени загрузки (напр. неверный порядок `--lua-init`,
  когда `init_vars` зовёт `tls_mod` до `zapret-antidpi`).
- **Не ловит** даже так: вызов несуществующей `--lua-desync` функции — он
  происходит по-пакетно в рантайме, а не на lua-init. От такого «тихого 0%»
  защищает статический инвариант `tests/test_nfqws_lua_map.py`, а не валидация.
  Не ловит и незаявленный blob (резолв тоже по-пакетно, см. §2).
- ⚠️ **`--intercept=0` не обязан завершаться сразу.** По эталону: «при
  наличии таймеров nfqws2 не выходит сразу, а исправно вызывает таймеры,
  пока не останется ни одного». Стратегия, чей `lua-init` заводит
  **периодический** таймер, не завершится никогда. Нас спасает
  `dry_run(strategy_args, timeout=8.0)` — такая стратегия падает по
  таймауту, а не вешает GUI. Если валидация внезапно упирается в таймаут на
  внешне корректной стратегии — искать `timer_set` в её `--lua-init`.
- Почему не `--dry-run`: по эталону он Lua НЕ загружает («Lua не проверяется»).
  `--intercept=0` — строгий супермножество: парсинг опций + файлы + lua-init.
- API:
...
**Остановка (важно с 1.0.4).** Наш `stop()` шлёт `SIGTERM` группе процессов
(`os.killpg`). До 1.0.4 `blockcheck2.sh` ловил только `INT`/`PIPE`/`HUP`, и
`SIGTERM` убивал его **посреди теста** — накатанные им временные
iptables/nft-правила и запущенный pktws оставались висеть, ломая
связность. С **1.0.4** скрипт трапит `PIPE HUP TERM QUIT` и на них
выполняет `unprepare_all` (снос своих правил + `ws_kill`). То есть наш
существующий `stop()` корректно чистит за собой **только на zapret2 ≥ 1.0.4**;
на более старых после аварийной остановки стоит проверить firewall вручную.
...
⚠️ **`PKTWS_EXTRA_POST_1` до 1.0.4 не работал** — в `strategy_append_extra_pktws()`
была опечатка: проверялась переменная `PKTWS_EXTRA_1`, а подставлялась
`PKTWS_EXTRA_POST_1`. Остальные `_2…_9` работали. Починено в 1.0.4. Если
GUI прокидывает post-параметры — на старых сборках первый слот молча терялся.

**BATCH-режим и возможности curl** (в master после 1.0.4, релиза пока нет):
если `ENABLE_HTTPS_TLS13=1` / `ENABLE_HTTP3=1`, а установленный curl их не
поддерживает, blockcheck2 теперь печатает `WARNING ! installed curl version
does not support …` и **сам выставляет флаг в 0**. Раньше в `BATCH=1` такие
тесты просто уходили в сплошной UNAVAILABLE. Наш парсер вывода должен быть
готов к обоим вариантам.
...
17. **Несовпадение версий lua и бинарника**: `Incompatible NFQWS2_COMPAT_VER`
    в логе — lua и nfqws2 из разных релизов. Сейчас актуальна compat **6**
    (zapret2 ≥ 1.0). См. §0.2 про то, что внутри одной compat-версии наш
    `asset_importer` перезаписывает upstream-lua.
18. **`rawpacket_queue failed !` в debug-логе**: переполнена 64-пакетная
    очередь реасма (§8.10) — реасм отменён, `l7payload` сброшен в `unknown`,
    стратегия на `tls_client_hello`/`quic_initial` перестала применяться на
    этом соединении.
19. **Незаявленный blob**: с 1.0.4 инстанс сносится в C-коде до Lua (§2) —
    в логе не будет даже `blob '…' unavailable` из Lua. Сверяться с
...
- Таймеры: `timer_set(name, func, period, oneshot, data)`, `timer_del(name)`,
  `timer_info(name)`, `timer_enum()` — подробно §19.5.
...
- Файлы: `--writable` создаст каталог, путь в `os.getenv("WRITABLE")`
  (до zapret2 1.0 — `--writeable` / `WRITEABLE`). Хелпер из `zapret-lib`
  для относительных путей — `writable_file_name(filename)` (до 1.0 —
  `writeable_file_name`).
- Сжатие потоком (`zstream`): `gzip_deflate(chunk[, expected_size])` и
  `gunzip_inflate(chunk)` — можно скармливать данные частями, каждая
  возвращает `(данные, признак_конца)`; финализация deflate — вызов с
  `nil`/`""`. При порче данных или нехватке памяти — `nil`.

Имена таймеров — хелперы `zapret-lib`:
`dis_timer_name(dis)` (src/dst IP + l4 + порты или icmp type:code, НЕ
гарантированно уникально) и `desync_timer_name(desync)` (имя инстанса +
`dis_timer_name` + номер пакета в conntrack по прямому направлению; без
`track` добавляются случайные символы). Второй считается уникальным на
desync+инстанс — годится как имя oneshot-таймера. Включение имени инстанса
добавили в 1.0.4.
...
### 19.5 Таймеры (появились в 0.9.5, доработаны в 1.0.1–1.0.3)

Единственный способ сделать что-либо асинхронно: `sleep`/задержек в Lua нет
принципиально — nfqws2 однопоточный, пауза вешает всю очередь NFQUEUE.

```lua
timer_set(name, func, period, oneshot, data)   -- создать/заместить
timer_del(name)                                 -- удалить
timer_info(name)  --> {name, func, oneshot, period, fires} | nil
timer_enum()      --> массив имён всех таймеров
```

- `name` — уникальное имя. Таймер с тем же именем **замещается**, отсчёт
  начинается заново.
- `func` — **с 1.0.3 это string ИЛИ сама function**. В случае строки функция
  ищется по имени в момент вызова `timer_set` и запоминается **по ссылке**
  (не как строка) — переопределение глобала позже уже не подействует.
- `period` — период в **миллисекундах**.
- `oneshot` — `true` = однократный (удаляется сам после вызова),
  `false` = периодический.
- `data` — произвольное значение, отдаётся таймер-функции; таблица удобна
  для состояния. Освобождается при удалении таймера.

Прототип: `function timer(name, data)`.

Правила, о которые легко споткнуться:

1. **`error` внутри таймер-функции = таймер принудительно удаляется.** Долбёжки
   по ошибке не будет — он просто тихо перестанет работать. Поэтому в
   таймер-функциях особенно важно не допускать error.
2. Таймеры вызываются **между блоками пакетов** (Linux/Windows принимают
   пакеты пачками), поэтому точность по времени приблизительная.
3. `timer_del` не прерывает уже исполняющийся вызов, но следующих не будет.
   Удаление таймера **изнутри его же функции** до 1.0.1 давало
   use-after-free — на ≥1.0.1 безопасно.
4. Имя для oneshot-таймера на поток удобно брать из
   `desync_timer_name(desync)` (см. §19.3).
5. В `--intercept=0` наличие таймеров откладывает выход процесса — см. §12.1.
