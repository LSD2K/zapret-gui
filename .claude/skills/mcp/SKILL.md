...
* **подписка обещается только там, где её можно исполнить.**
  ...
  вызов пришёл по транспорту с каналом (legacy-SSE), иначе `false`. На
...
| `tests/test_mcp_stdio.py` | мост: построчно, батч, мусор → `-32700` и мост жив, EOF завершает, в stdout только JSON, служебное в stderr, прокси-режим и его отказы | при правке моста |
