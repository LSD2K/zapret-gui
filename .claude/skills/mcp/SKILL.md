...
* **Домены и публичные адреса маскируются при РЕНДЕРЕ** (`_Masker`,
  ...
  (окончание из `_FILE_SUFFIXES`), пути модулей (`_MODULE_HEADS`),
  адреса апстримов (`_KEEP_DOMAINS`). Метки в `repro` делают команду
  невоспроизводимой — рядом с ней строка «замените метки».
