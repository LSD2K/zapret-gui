...
| Роут | Что делает |
|---|---|
| `GET /api/mcp/ui/state` | **всё состояние страницы одним ответом**: `info` (то же, что `/api/mcp/info`), `access`, `audit`, `experiment`, `code`, `shell` |
...
* **`--writable` выдаёт `NFQWSManager.compose_command`** — любой
  стратегии, где есть инстанс `pcap` и нет своего `--writable`
  (`--writeable` до zapret2 1.0 тоже считается своим). Каталог —
  `lua_capture.writable_dir()` (`/tmp/zapret-gui-writable`, tmpfs:
  дамп нужен на минуту), `start()` создаёт его и отдаёт `nfqws.user`
  заранее — на существующем каталоге nfqws2 права не меняет. Без
  флага `pcap` с относительным именем пишет в cwd процесса, куда под
...
| `tests/test_mcp_page.js` | сама страница в `node:vm` поверх мини-DOM: `render`/`destroy`, одно состояние одним запросом, токен точками, скрытые блоки 6–8, предупреждения из i18n, `mcpServers` без поля `type` | при правке разметки страницы |
