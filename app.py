...
def _apply_saved_strategy_on_boot():
    ...
    def _do_apply():
        try:
            ...
            # То же и с дедмен-свитчами shell (MCP): команда, которой
            # модель могла отрезать доступ, заряжает откат таймером в
            # памяти. Процесс перезапустился — таймера нет, а откат
            # нужен как раз тогда. Снимок лежит на диске: просроченные
            # исполняем сразу, живые перезаряжаем.
            try:
                from core.shell_exec import recover_guards
                recover_guards(source="autostart")
            except Exception as e:
