...
def build_options() -> dict:
    ...
    return {
        ...
        **_front_options(configs),
    }


def _front_options(configs) -> dict:
    from core.singbox_fakeip_front import form_options
    return form_options(configs)
...
def build_and_save(*, name: str = "fakeip", proxy_link: str = "",
    ...
    нужны).
    """
    front = str(front_dns or "engine").strip().lower()
    if front != "engine":
        from core.singbox_fakeip_front import build_and_save_front
        return build_and_save_front(
            front, name=name, proxy_link=proxy_link,
            proxy_config=proxy_config, hostlists=hostlists, domains=domains,
            direct_dns=direct_dns, tun_iface=tun_iface, stack=stack,
            dns_port=dns_port, **front_kw)
    if dns_port is None:
        dns_port = 1153
    if direct_dns is None:
        direct_dns = "local"
    ...
    if front_err:
        return {"ok": False, "error": front_err}
