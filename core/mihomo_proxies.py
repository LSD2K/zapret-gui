...
def safe_mutate(text: str, mutate_fn) -> dict:
    ...
    """
    if not has_pyyaml():
        # Текст обезличен намеренно: функция общая для удаления прокси из
        # таблицы и для подключения opera-proxy, а «удаление» в ответе на
        # нажатие «подключить» читается как чужая ошибка. Свою формулировку
        # вызывающий подставляет сам по флагу needs_pyyaml.
        return {"ok": False, "needs_pyyaml": True, "error":
                "Эта правка конфига требует модуля PyYAML: без него "
                "перезапись сложного YAML повредила бы его. Установите "
                "PyYAML (python3-yaml) или правьте конфиг вручную на "
                "странице mihomo."}
    try:
        cfg = parse_yaml(text)
