...
Состав пакета:
  ...
  ``server.py``      — диспетчер JSON-RPC (методы протокола);
  ``registry.py``    — реестр: ``@tool``, вызов, форма ответа;
  ``permissions.py`` — разрешения и граница записи настроек;
  ``redact.py``      — вырезание секретов из результата инструмента;
