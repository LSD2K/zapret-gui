...
PROMPTS = (
    ...
    {
        "name": "report_problem",
        "title": "Составить отчёт об ошибке в самом GUI",
        "description": ("Turn a GUI bug (tool crash, result contradicting "
                        "the description) into an issue draft with code "
                        "location and repro. / Составить черновик issue "
                        "об ошибке zapret-gui."),
        "arguments": [
            {"name": "what", "description":
             "Что пошло не так: инструмент и что он сделал.",
             "required": False},
        ],
        "intro": ("Задача: превратить замеченную ошибку GUI ({what}) в "
                  "черновик issue, по которому разработчик найдёт место "
                  "в коде. Сначала решите, ошибка ли это GUI: «сайт не "
                  "открывается», «стратегия не помогла», «туннель не "
                  "поднялся из-за конфига» — это состояние сети и "
                  "устройства, для них issue не пишут. Пишут, когда "
                  "инструмент упал (в ответе есть crash_id), когда ответ "
                  "противоречит описанию инструмента или скилу, когда "
                  "документация расходится с поведением."),
        "steps": (
            ("issue_draft_list", "посмотреть, нет ли уже черновика про "
                                 "это (и падений без черновика в "
                                 "`crashes_without_draft`) — повтор "
                                 "склеится сам, но лишний вызов не нужен"),
            ("audit_list", "восстановить шаги: какие вызовы и с какими "
                           "аргументами привели к ошибке"),
            ("logs_tail", "строки лога вокруг ошибки (`level=\"WARNING\"`) "
                          "— если в них есть объяснение, оно пойдёт в "
                          "`evidence`"),
            ("issue_draft", "составить черновик: `title` одной строкой, "
                            "`kind`, `tool`, `crash_id` (если был), "
                            "`expected` со ссылкой на описание "
                            "инструмента, `actual`, `steps`. Файл, строку, "
                            "трассировку и команду воспроизведения сервер "
                            "допишет сам — не выдумывайте их"),
        ),
        "outro": ("Черновик не отправляется сам: скажите человеку, что он "
                  "лежит на странице MCP (блок «Черновики issue»), и дайте "
                  "ссылку `open_on_github`. Домены и публичные адреса в "
                  "нём замаскированы — `include_targets=true` только если "
                  "без конкретного домена ошибку не воспроизвести."),
    },
)
...
def get_prompt(name, arguments=None) -> dict:
    ...
    if not values.get("what"):
        values["what"] = "см. последние вызовы"
