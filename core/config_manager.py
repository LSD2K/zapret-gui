...
DEFAULT_CONFIG = {
    ...
    "mcp": {
        ...
        "permissions": {
            ...
            # S17: снимает маскировку в ответе (по явному `raw: true`)
            # и пускает запись в поля, похожие на секрет. Своих
            # инструментов не открывает.
            "secrets": False,
        },
        "limits": {
            ...
            # Сколько `job_wait` держит вызов, ожидая конца тяжёлой
            # операции (S17). Потолок — заведомо меньше
            # tool_timeout_sec: ожидание, которое само упирается в
            # таймаут клиента, хуже опроса.
            "wait_sec": 45,
            "max_wait_sec": 90,
        },
        "probes": {
            # Активные пробы (разрешение `probes`): сколько целей за
            # вызов, сколько повторов на цель, сколько ждать каждую
            # сетевую операцию и сколько всего времени отдавать одному
            # вызову. Потолки нужны не ради нагрузки на роутер, а ради
            # провайдера: сотня доменов подряд с одного адреса — это
            # заметный след, а не диагностика.
            ...
            "budget_sec": 60,              # суммарный бюджет одного вызова
        ...
        "capture": {
            # Снифер после nfqws2 (S17, разрешение `probes`): смотрим,
            # ЧТО реально ушло в сеть, а не что собирались отправить.
            # Потолки жёсткие: дамп на роутере со 128 МБ — это и файл, и
            # память, и заметный провайдеру след.
            "max_packets": 200,            # пакетов за один прогон
            "max_seconds": 30,             # и не дольше этого
            "snaplen": 256,                # байт пакета (SNI влезает)
            "keep_file": False,            # оставлять ли pcap на диске
        },
        "shell": {
    ...
    # --- Встроенный агент (S18) ---
    # Клиент к OpenAI-совместимому API (LM Studio, Ollama, llama.cpp)
    # ПРЯМО НА СТРАНИЦЕ: «подбери стратегию сам» без внешнего клиента и
    # без токена. Ходит теми же инструментами и под теми же
    # разрешениями, что MCP (`mcp.permissions`) — своих не имеет.
    #
    # Отдельный флаг, потому что это отдельная дверь: выключенный
    # MCP-транспорт агента не выключает, и наоборот. По умолчанию
    # выключен и никуда не ходит.
    "agent": {
        "enabled": False,
        # LM Studio — http://127.0.0.1:1234/v1, Ollama —
        # http://127.0.0.1:11434/v1. К локальным адресам ходим МИМО
        # прокси окружения: HTTPS_PROXY на роутере обычно настроен на
        # обход блокировок, и запрос к 127.0.0.1 через него не пройдёт.
        "base_url": "http://127.0.0.1:1234/v1",
        "api_key": "",                 # локальным серверам не нужен
        "model": "",                   # пусто = что отдаст сервер
        "max_steps": 12,               # потолок шагов «модель → вызов»
        "timeout_sec": 120,            # один запрос к модели
        "temperature": 0.2,
        # scenarios — только инструменты готовых сценариев (~30):
        # 114 объявлений это 15k токенов в каждом запросе, и локальная
        # модель на 8B захлебнётся до первого вызова. all — весь реестр.
        "tools": "scenarios",
        "tool_result_chars": 6000,     # сколько вывода уезжает обратно
    },
}
