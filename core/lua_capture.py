...
def writable_args(argv, directory: str = "") -> list:
    ...
    Нужен он ровно тогда, когда в argv есть ``pcap``, а каталог не задан
    самой стратегией: её выбор главнее нашего умолчания.
    """
...
def prepare_dir(directory: str = "", user: str = "") -> str:
    ...
    nfqws2 сбрасывает права до ``--user`` (обычно ``nobody``) — каталог,
    созданный root'ом, для записи ему закрыт. Движок умеет создать его
    сам, но на уже существующем каталоге прав не меняет, поэтому
    готовим сами. Не вышло — не беда: запись упадёт внутри lua, и это
    будет видно в логе движка, а старт стратегии не сорвётся.
    """
